<?php

return [
    'meta' => [
        'title' => 'Denetfils : épicerie agroalimentaire haïtienne en Europe',
        'description' => 'DEN & FILS propose des produits alimentaires haïtiens et caribéens : pâte de djon djon, piment Edenne, épis SELAKAY, boissons, sauces et recettes authentiques.',
    ],
    'announcement' => 'Livraison gratuite dès 35€ · Poto mitan kizin ou · Produits haïtiens authentiques',
    'announcements' => [
        'Offre spéciale : livraison gratuite à partir de 35€.',
        'DEN & FILS — Poto mitan kizin ou, la pierre angulaire de votre cuisine.',
        'Produits authentiques, recettes familiales et saveurs haïtiennes livrées chez vous.',
    ],
    'nav' => [
        'promise' => 'Poto mitan kizin ou',
        'home' => 'Accueil',
        'about' => 'Notre histoire',
        'shop' => 'Boutique',
        'blog' => 'Recettes',
        'checkout' => 'Infos utiles',
    ],
    'locale' => ['fr' => 'Français', 'en' => 'Anglais', 'switch_to' => 'Passer en :language.'],
    'theme' => ['label' => 'Choix du thème.', 'toggle' => 'Changer le thème.', 'light' => 'Clair', 'dark' => 'Sombre'],
    'hero' => [
        'eyebrow' => 'Saveurs haïtiennes & tropicales',
        'title_1' => 'Le marché haïtien',
        'title_accent' => 'livré',
        'title_2' => 'chez vous.',
        'body' => 'Retrouvez vos essentiels haïtiens et caribéens : sauces, épices, boissons, aides culinaires, produits frais et coffrets gourmands.',
        'primary_cta' => 'Découvrir la boutique',
        'secondary_cta' => 'Voir les best-sellers',
        'chef_pick' => 'Sélection de la maison',
        'chef_title' => 'Coffret créole #001',
        'discount' => '-15% 1ère cmd',
        'image_alt' => 'Panier de produits tropicaux et haïtiens',
        'trust' => [
            ['title' => 'Livraison rapide', 'body' => 'Offerte dès 35€ selon le panier.'],
            ['title' => 'Paiement sécurisé', 'body' => 'Carte, PayPal et données chiffrées.'],
            ['title' => 'Produits authentiques', 'body' => 'Djondjon, piment Edenne, épis et sauces.'],
        ],
    ],
    'slider' => [
        'badge' => 'Sélection Denetfils',
        'items' => [
            ['label' => 'Aides culinaires', 'title' => 'Pâte de djon djon & piment Edenne pour redonner vie à vos plats.', 'body' => 'Deux essentiels de la cuisine haïtienne : le djondjon pour parfumer les mets et le piment Edenne pour une chaleur maîtrisée.', 'image' => 'https://images.unsplash.com/photo-1596040033229-a9821ebd058d?auto=format&fit=crop&w=1400&q=85'],
            ['label' => 'Épicerie haïtienne', 'title' => 'Céréales, féculents, sauces et boissons pour une cuisine généreuse.', 'body' => 'Des produits soigneusement sélectionnés pour préserver le goût, la tradition et la qualité dans chaque panier.', 'image' => 'https://images.unsplash.com/photo-1542838132-92c53300491e?auto=format&fit=crop&w=1400&q=85'],
            ['label' => 'Recettes & transmission', 'title' => 'Une maison familiale qui fait voyager les saveurs haïtiennes.', 'body' => 'DEN & FILS prolonge un héritage culinaire transmis depuis plusieurs générations, entre mémoire familiale et exigence moderne.', 'image' => 'https://images.unsplash.com/photo-1506806732259-39c2d0268443?auto=format&fit=crop&w=1400&q=85'],
        ],
    ],
    'about' => [
        'eyebrow' => 'À propos de DEN & FILS',
        'title' => 'Le goût vrai, sans détour.',
        'body' => 'DEN & FILS est née d’une histoire de famille transmise avec amour depuis plus de cinq générations. La maison met en valeur des recettes ancestrales haïtiennes, notamment autour de la pâte de djondjon, du piment Edenne et des aides culinaires SELAKAY.',
        'image_alt' => 'Produits alimentaires haïtiens et ingrédients de cuisine.',
        'image_caption' => 'Chaque produit porte une mémoire familiale, un savoir-faire et une culture culinaire.',
        'story_title' => 'Notre histoire',
        'story_body' => 'Fondée sur les recettes des aïeules, des arrière-arrière-grands-mères et de la mère Edenne, DEN & FILS préserve les gestes, les goûts et les traditions qui ont traversé les générations. Chaque pot raconte une cuisine haïtienne authentique et généreuse.',
        'vision_title' => 'Notre vision',
        'vision_body' => 'Basée en France, DEN & FILS supervise l’importation, la sélection, la fabrication et la distribution. La société porte les marques LAKAY, SELAKAY et KETIKOTI avec une volonté claire : protéger l’authenticité des recettes et garantir une qualité constante.',
        'reach_title' => 'Présence internationale',
        'reach_items' => ['Europe : distribution via des grossistes spécialisés.', 'Canada : partenariats commerciaux fiables.', 'États-Unis : présence via SELAKAY LLC et DEN & FILS LLC.'],
        'commitment_title' => 'Notre engagement',
        'commitment_body' => 'DEN & FILS travaille avec des coopératives locales sélectionnées pour la qualité, le respect humain et le développement local. L’entreprise affirme aussi une mission sociale : soutenir l’éducation des enfants en Haïti, particulièrement dans les zones vulnérables.',
        'points' => [
            ['eyebrow' => 'Authenticité', 'title' => 'Recettes familiales', 'body' => 'Des produits inspirés d’un héritage culinaire haïtien transmis de génération en génération.'],
            ['eyebrow' => 'Qualité', 'title' => 'Sélection exigeante', 'body' => 'Ingrédients, recettes et partenaires choisis pour préserver le goût et la confiance.'],
            ['eyebrow' => 'Transmission', 'title' => 'Culture vivante', 'body' => 'Une marque qui ne vend pas seulement des condiments, mais une histoire et une mémoire.'],
        ],
    ],
    'contact' => [
        'eyebrow' => 'Informations officielles',
        'title' => 'Contact & coordonnées',
        'company' => 'DEN et FILS',
        'address' => '4 Rue des Grands Champs, 51520 Saint Martin sur le Pré, France',
        'phone' => '+33 6 95 73 73 90',
        'email' => 'contact@denetfils.fr',
        'vat' => 'N° TVA : FR88 939445672',
    ],
    'stats' => [
        ['value' => '35€', 'label' => 'Livraison gratuite à partir du panier minimum.'],
        ['value' => '5+', 'label' => 'Générations de transmission culinaire familiale.'],
        ['value' => '3', 'label' => 'Présence commerciale : Europe, Canada et États-Unis.'],
    ],
    'categories' => [
        'eyebrow' => 'Rayons',
        'title' => 'Explorez le marché',
        'body' => 'Choisissez votre rayon et retrouvez les saveurs du pays en quelques clics.',
        'count' => ':count produits',
    ],
    'offers' => [
        'main_eyebrow' => 'Nouveautés',
        'main_title' => 'Les arrivages du moment',
        'body' => 'Des produits sélectionnés chaque semaine pour retrouver les vraies saveurs du pays.',
        'cta' => 'Voir les arrivages',
        'cards' => [
            ['eyebrow' => 'Drop hebdo', 'title' => 'Récolte de la semaine', 'body' => 'Mangues, ignames, piments, sauces et plus encore.'],
            ['eyebrow' => 'Épicerie', 'title' => 'Riz & pois rouges', 'body' => 'Une sélection simple pour composer vos plats du quotidien.'],
            ['eyebrow' => 'Boissons', 'title' => 'Nectars tropicaux', 'body' => 'Des jus et boissons pour accompagner les repas créoles.'],
        ],
    ],
    'spotlight' => ['eyebrow' => 'Sélection', 'title' => 'Produits recommandés', 'cta' => 'Voir le catalogue'],
    'blog' => [
        'eyebrow' => 'Cuisine',
        'title' => 'Recettes & inspirations',
        'body' => 'Découvrez comment utiliser nos produits dans des plats simples, généreux et pleins de goût.',
        'posts' => [
            ['date' => '16/12/2025', 'category' => 'Nouveauté', 'title' => "L'Épice que votre cuisine attendait", 'body' => 'DEN & FILS présente une épice complète inspirée de l’héritage haïtien, pensée pour simplifier la cuisine du quotidien.'],
            ['date' => '20/11/2025', 'category' => 'Histoire', 'title' => 'Le goût suffit-il pour faire vivre une légende ? Histoire de la pâte de Djondjon', 'body' => 'Un récit autour d’Edenne, du djon djon et de la transmission familiale qui a donné naissance à la pâte moderne.'],
            ['date' => '19/11/2025', 'category' => 'Marque', 'title' => 'Pourquoi tant de personnes choisissent Denetfils ?', 'body' => 'Un article qui explique la différence Denetfils : authenticité, histoire, goût et confiance.'],
            ['date' => '17/11/2025', 'category' => 'Recette', 'title' => 'Poisson épicé au piment Edenne & pâte de djon djon', 'body' => 'Une idée recette pour associer piment Edenne et djon djon dans un plat généreux.'],
            ['date' => '05/11/2025', 'category' => 'Recette', 'title' => 'Recette de riz Djondjon', 'body' => 'Un classique haïtien pour cuisiner le riz avec la pâte de djon djon SELAKAY.'],
        ],
    ],
    'products' => [
        'eyebrow' => 'Boutique',
        'title' => 'Des produits authentiques, prêts à rejoindre votre cuisine.',
        'body' => 'Image, origine, disponibilité, prix et bouton panier : chaque carte garde les informations essentielles.',
        'cta' => 'Ajouter',
        'stock_label' => ':count en stock',
        'api_error' => 'Le catalogue est momentanément indisponible. Veuillez réessayer dans quelques instants.',
        'empty' => 'Aucun produit n’est disponible pour le moment.',
    ],
    'filters' => [
        'search' => 'Recherche',
        'search_placeholder' => 'Rechercher un produit, une origine ou une référence.',
        'category' => 'Catégorie',
        'all_categories' => 'Toutes les catégories',
        'sort' => 'Tri',
        'sort_options' => ['default' => 'Sélection Denetfils', 'latest' => 'Plus récents', 'price_asc' => 'Prix croissant', 'price_desc' => 'Prix décroissant'],
        'apply' => 'Rechercher',
        'reset' => 'Réinitialiser',
    ],
    'cart' => [
        'title' => 'Panier',
        'subtitle' => 'Vos produits sélectionnés sont conservés sur cet appareil.',
        'close' => 'Fermer le panier',
        'loading' => 'Chargement du panier...',
        'empty' => 'Votre panier est vide.',
        'remove' => 'Retirer cet article',
        'total' => 'Total',
        'checkout_later' => 'Passer la commande',
        'api_error' => 'Impossible de mettre à jour le panier.',
        'expired' => 'Ce panier n’est plus disponible. Un nouveau panier sera créé au prochain ajout.',
        'empty_total' => '0,00 EUR',
    ],
    'account' => [
        'nav' => 'Compte',
        'api_error' => 'Le service compte client est momentanément indisponible. Veuillez réessayer dans quelques instants.',
        'auth' => [
            'login_title' => 'Connectez-vous à votre compte.',
            'login_intro' => 'Accédez à votre profil et à vos adresses de livraison européennes enregistrées.',
            'register_title' => 'Créez votre compte client.',
            'register_intro' => 'Créez un compte sécurisé avant la commande, avec consentements RGPD et pays européens supportés.',
            'login_success' => 'Vous êtes connecté.',
            'register_success' => 'Votre compte a été créé.',
            'logout_success' => 'Vous êtes déconnecté.',
            'required' => 'Veuillez vous connecter pour accéder à votre compte.',
            'expired' => 'Votre session a expiré. Veuillez vous reconnecter.',
            'email' => 'Adresse email',
            'password' => 'Mot de passe',
            'password_confirmation' => 'Confirmer le mot de passe',
            'first_name' => 'Prénom',
            'last_name' => 'Nom',
            'phone' => 'Téléphone',
            'country' => 'Pays',
            'sign_in' => 'Se connecter',
            'create_account' => 'Créer le compte',
            'no_account' => 'Pas encore de compte ?',
            'has_account' => 'Vous avez déjà un compte ?',
            'go_register' => 'Créer un compte',
            'go_login' => 'Se connecter',
            'privacy_consent' => 'J’accepte la politique de confidentialité.',
            'terms_consent' => 'J’accepte les conditions d’utilisation.',
            'marketing_consent' => 'J’accepte de recevoir les offres et nouveautés par email.',
        ],
        'overview' => [
            'eyebrow' => 'Espace client',
            'fallback_name' => 'client Denetfils',
            'title' => 'Bonjour :name',
            'body' => 'Retrouvez vos informations de compte, vos adresses et vos dernières commandes synchronisées avec l’API client.',
            'orders' => 'Commandes',
            'addresses' => 'Adresses',
            'default_shipping' => 'Livraison principale',
            'no_default_shipping' => 'Aucune adresse principale',
        ],
        'profile' => [
            'title' => 'Mon compte',
            'intro' => 'Gérez les informations utilisées pour le support client, les factures et les futures commandes.',
            'section_title' => 'Profil',
            'updated' => 'Votre profil a été mis à jour.',
            'preferred_locale' => 'Langue préférée',
            'timezone' => 'Fuseau horaire',
            'save' => 'Enregistrer le profil',
            'status' => 'Statut',
            'role' => 'Rôle',
            'logout' => 'Se déconnecter',
        ],
        'orders' => [
            'eyebrow' => 'Historique',
            'title' => 'Commandes récentes',
            'intro' => 'Les dernières commandes sont récupérées depuis le compte client sécurisé.',
            'empty' => 'Aucune commande n’est encore rattachée à ce compte.',
            'number' => 'Commande',
            'payment' => 'Paiement',
            'fulfillment' => 'Préparation',
            'items' => ':count article|:count articles',
            'placed_at' => 'Passée le :date',
            'more_items' => 'articles en plus',
        ],
        'addresses' => [
            'title' => 'Adresses',
            'intro' => 'Conservez des adresses complètes de facturation et de livraison pour le checkout européen et la logique TVA.',
            'new_title' => 'Ajouter une adresse',
            'empty' => 'Aucune adresse n’est encore enregistrée.',
            'created' => 'Adresse créée.',
            'updated' => 'Adresse mise à jour.',
            'deleted' => 'Adresse supprimée.',
            'type' => 'Type',
            'shipping' => 'Livraison',
            'billing' => 'Facturation',
            'label' => 'Libellé',
            'recipient_name' => 'Nom du destinataire',
            'company' => 'Société',
            'street_line_1' => 'Adresse ligne 1',
            'street_line_2' => 'Adresse ligne 2',
            'postal_code' => 'Code postal',
            'city' => 'Ville',
            'region' => 'Région',
            'country_code' => 'Pays',
            'phone' => 'Téléphone',
            'is_default' => 'Adresse par défaut pour ce type',
            'save' => 'Enregistrer l’adresse',
            'update' => 'Mettre à jour',
            'delete' => 'Supprimer',
            'default_badge' => 'Par défaut',
        ],
    ],
    'product' => [
        'back' => 'Retour au catalogue',
        'details' => 'Informations produit',
        'details_body' => 'Une fiche claire pour consulter les informations utiles avant l’achat.',
        'category' => 'Catégorie',
        'sku' => 'Référence',
        'stock' => 'Stock',
        'weight' => 'Poids',
        'price' => 'Prix',
        'variant' => 'Variante',
        'available' => ':count disponibles',
        'related_title' => 'Produits associés',
        'shipping_note' => 'Livraison, TVA et paiement sécurisé intégrés au parcours de commande.',
    ],
    'checkout' => [
        'eyebrow' => 'Infos utiles',
        'title' => 'Commander chez Denetfils.',
        'body' => 'Retrouvez les informations qui aident à commander sereinement : livraison, disponibilité et accompagnement client.',
        'steps' => [
            ['number' => '01', 'title' => 'Livraison', 'body' => 'Livraison offerte dès 35€ et informations visibles avant validation.'],
            ['number' => '02', 'title' => 'Disponibilité', 'body' => 'Les produits indiquent leur stock pour éviter les mauvaises surprises.'],
            ['number' => '03', 'title' => 'Assistance', 'body' => 'Contact par téléphone, email ou WhatsApp selon vos besoins.'],
        ],
    ],
    'utility_pages' => [
        'delivery' => [
            'eyebrow' => 'Liens utiles', 'title' => 'Livraison', 'intro' => 'Retrouvez les informations essentielles pour préparer votre commande DEN & FILS.',
            'sections' => [
                ['title' => 'Livraison gratuite dès 35€', 'body' => 'L’offre spéciale de la boutique indique une livraison gratuite à partir de 35€ de commande. Les conditions exactes sont affichées dans le parcours d’achat selon l’adresse et le panier.'],
                ['title' => 'Adresse de livraison', 'body' => 'Le client doit renseigner une adresse complète et valide. Les frais et possibilités de livraison sont confirmés avant validation de la commande.'],
                ['title' => 'Assistance', 'body' => 'Pour toute question, DEN & FILS est joignable par téléphone au +33 6 95 73 73 90 ou par email à contact@denetfils.fr.'],
            ],
        ],
        'legal' => [
            'eyebrow' => 'Liens utiles', 'title' => 'Mentions légales', 'intro' => 'Informations officielles de la société DEN et FILS.',
            'sections' => [
                ['title' => 'Éditeur du site', 'body' => 'DEN et FILS, 4 Rue des Grands Champs, 51520 Saint Martin sur le Pré, France.'],
                ['title' => 'Contact', 'body' => 'Téléphone : +33 6 95 73 73 90. Email : contact@denetfils.fr.'],
                ['title' => 'TVA', 'body' => 'N° TVA : FR88 939445672.'],
            ],
        ],
        'terms' => [
            'eyebrow' => 'Liens utiles', 'title' => 'Conditions d’utilisation', 'intro' => 'Règles générales d’utilisation de la boutique DEN & FILS.',
            'sections' => [
                ['title' => 'Utilisation du site', 'body' => 'Le visiteur utilise le site pour consulter les produits, ajouter des articles au panier et accéder aux informations commerciales de DEN & FILS.'],
                ['title' => 'Informations produits', 'body' => 'Les descriptions, prix et disponibilités peuvent évoluer. Le client doit vérifier le récapitulatif de commande avant validation.'],
                ['title' => 'Newsletter et données', 'body' => 'L’inscription à la newsletter permet de recevoir les offres, nouveautés et actualités. Le client peut se désinscrire à tout moment.'],
            ],
        ],
        'payment' => [
            'eyebrow' => 'Liens utiles', 'title' => 'Paiement sécurisé', 'intro' => 'DEN & FILS met en avant une commande simple et sécurisée.',
            'sections' => [
                ['title' => 'Commande sécurisée', 'body' => 'Le paiement intervient dans le parcours de commande après vérification du panier, de la livraison et du total à régler.'],
                ['title' => 'Vérification du panier', 'body' => 'Le client peut contrôler les produits, quantités, prix et frais avant de finaliser son achat.'],
                ['title' => 'Support client', 'body' => 'En cas de doute avant paiement, le client peut contacter DEN & FILS par téléphone ou par email.'],
            ],
        ],
    ],
    'footer' => [
        'line' => 'DEN & FILS — produits alimentaires haïtiens, tradition familiale et livraison en Europe.',
        'products_title' => 'Boutique',
        'promotions' => 'Promotions',
        'new_products' => 'Nouveaux produits',
        'best_sellers' => 'Meilleures ventes',
        'useful_links_title' => 'Liens utiles',
        'delivery' => 'Livraison',
        'legal' => 'Mentions légales',
        'terms' => "Conditions d'utilisation",
        'secure_payment' => 'Paiement sécurisé',
        'information_title' => 'Informations',
    ],
];
