...
@section('content')
    <section class="soft-grid px-4 py-14 dark:bg-ink sm:px-8 lg:py-20">
        <div class="mx-auto max-w-5xl">
            ...
            <h1 class="brand-display mt-4 text-5xl uppercase text-forest dark:text-meadow sm:text-7xl">
                {{ $content['title'] }}
            </h1>
            <p class="mt-6 max-w-3xl text-base font-semibold leading-8 text-cocoa/70 dark:text-cream/70">
                {{ $content['intro'] }}
            </p>
        </div>
    </section>
        ...
        <div class="mx-auto grid max-w-5xl gap-5">
            @foreach ($content['sections'] as $section)
                <article class="utility-section bg-linen dark:bg-white/5">
                    <p class="section-kicker">{{ str_pad((string) $loop->iteration, 2, '0', STR_PAD_LEFT) }}</p>
                    <h2 class="mt-3 text-2xl font-black text-forest dark:text-meadow">{{ $section['title'] }}</h2>
    ...
    <section class="bg-linen px-4 py-14 dark:bg-[#163319] sm:px-8 lg:py-16">
        <div class="utility-section mx-auto grid max-w-5xl gap-6 lg:grid-cols-[0.85fr_1.15fr]">
            <div>
