...
@section('content')
    <section class="soft-grid px-5 pb-14 pt-28 dark:bg-ink sm:px-8 lg:pb-20 lg:pt-32">
        <div class="mx-auto max-w-7xl">
            <a href="{{ route('home.localized', ['locale' => $locale]) }}#products" class="inline-flex items-center rounded-full border border-cocoa/10 bg-white/75 px-4 py-2 text-sm font-bold text-cocoa/70 transition hover:border-terracotta/40 hover:text-terracotta dark:border-white/10 dark:bg-white/5 dark:text-cream/70">
            ...
            <div class="mt-8 grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
                ...
                <div class="lg:sticky lg:top-28">
                    <p class="text-sm font-bold uppercase tracking-[0.2em] text-sage dark:text-cream/60">
                        {{ $product['origin'] }}
                    </p>
                    ...
                    <aside class="glass-panel mt-7 rounded-[1.6rem] p-5" x-data="{ variantId: @js($product['variants'][0]['id'] ?? null) }">
                        <div class="flex items-start justify-between gap-5">
                            <div>
                                <p class="theme-subtle text-xs font-bold uppercase tracking-[0.2em] text-sage dark:text-cream/60">{{ __('home.product.price') }}</p>
                                <p class="theme-title mt-2 text-4xl font-extrabold text-terracotta dark:text-cream">{{ $product['formatted_price'] }}</p>
                            </div>
                            <span class="rounded-full bg-sage/10 px-3 py-2 text-xs font-bold text-leaf dark:bg-white/10 dark:text-cream">
    ...
    <section class="theme-band-soft bg-white px-5 py-14 dark:bg-[#211914] sm:px-8">
        <div class="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[1fr_0.8fr]">
            ...
            <dl class="premium-card grid gap-4 bg-linen p-6 dark:bg-white/5 sm:grid-cols-2">
                <div>
                    <dt class="theme-subtle text-xs font-bold uppercase tracking-[0.18em] text-sage dark:text-cream/60">{{ __('home.product.category') }}</dt>
                    <dd class="theme-title mt-2 text-sm font-extrabold text-cocoa dark:text-cream">{{ $product['category']['name'] ?? '-' }}</dd>
                </div>
                <div>
                    <dt class="theme-subtle text-xs font-bold uppercase tracking-[0.18em] text-sage dark:text-cream/60">{{ __('home.product.sku') }}</dt>
                    <dd class="theme-title mt-2 text-sm font-extrabold text-cocoa dark:text-cream">{{ $product['sku'] }}</dd>
                </div>
                <div>
                    <dt class="theme-subtle text-xs font-bold uppercase tracking-[0.18em] text-sage dark:text-cream/60">{{ __('home.product.stock') }}</dt>
                    <dd class="theme-title mt-2 text-sm font-extrabold text-cocoa dark:text-cream">{{ $product['stock_quantity'] }}</dd>
                </div>
                <div>
                    <dt class="theme-subtle text-xs font-bold uppercase tracking-[0.18em] text-sage dark:text-cream/60">{{ __('home.product.weight') }}</dt>
