<section id="products" class="theme-band-soft surface-transition bg-linen px-4 py-14 dark:bg-[#163319] sm:px-8 lg:py-20">
    <div class="mx-auto max-w-7xl">
        <div class="mb-8 flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
            <div>
                <p class="section-kicker">{{ __('home.products.eyebrow') }}</p>
                <h2 class="section-title mt-3 max-w-4xl">{{ __('home.products.title') }}</h2>
            </div>
            <p class="section-copy">{{ __('home.products.body') }}</p>
        </div>

        <button type="button" class="mb-3 flex min-h-[48px] w-full items-center justify-between rounded-2xl border border-leaf/10 bg-white px-4 py-3 text-sm font-black text-cocoa shadow-sm dark:border-white/10 dark:bg-white/5 dark:text-cream md:hidden" wire:click="$toggle('filtersOpen')">
            <span>{{ __('home.filters.apply') }}</span>
            <span class="text-forest">{{ $filtersOpen ? '−' : '+' }}</span>
        </button>
                ...
                <article class="market-card group overflow-hidden bg-white dark:bg-white/5" wire:key="product-card-{{ $product['id'] }}">
                    <a href="{{ route('products.show', ['locale' => $locale, 'slug' => $product['slug']]) }}" class="relative block overflow-hidden bg-cream" wire:navigate>
                        @if (! empty($primaryImage['url']))
                            <img class="product-card-image" src="{{ $primaryImage['url'] }}" alt="{{ $primaryImage['alt_text'] ?? $product['name'] }}" width="{{ $primaryImage['width'] ?? 600 }}" height="{{ $primaryImage['height'] ?? 450 }}" loading="{{ $primaryImage['loading'] ?? 'lazy' }}" decoding="async">
                        @else
                            <div class="grid h-60 place-items-center bg-sunshine/35 text-4xl font-black text-forest sm:h-72">DF</div>
                        @endif
                        <div class="absolute left-4 top-4 flex flex-wrap gap-2">
                            <span class="badge-tropical bg-forest text-cream">{{ $locale === 'fr' ? 'Sélection' : 'Selection' }}</span>
                            @if (! empty($product['origin']))
                                <span class="badge-tropical bg-cream/95 text-forest shadow-sm backdrop-blur">{{ $product['origin'] }}</span>
                            @endif
                        </div>
                    </a>
                    <div class="p-5">
                        ...
                        <h3 class="mt-2 min-w-0 text-2xl font-black leading-tight text-forest">
                        ...
                        <div class="mt-4 flex flex-wrap items-center justify-between gap-2 text-xs font-bold text-cocoa/55 dark:text-cream/55">
                            <span class="text-sunshine">★★★★★</span>
                            <span>{{ $ratingValue }} · {{ $reviewCount }} {{ $locale === 'fr' ? 'avis' : 'reviews' }}</span>
                        </div>
                        <div class="mt-3 flex items-center gap-2 text-xs font-black uppercase tracking-wide {{ $isAvailable ? 'text-forest dark:text-meadow' : 'text-coral' }}">
                            <span class="h-2 w-2 rounded-full {{ $isAvailable ? 'bg-forest dark:bg-meadow' : 'bg-coral' }}"></span>
                            <span>{{ $isAvailable ? __('home.products.stock_label', ['count' => $product['stock_quantity']]) : ($locale === 'fr' ? 'Rupture de stock' : 'Out of stock') }}</span>
                        </div>
                        <div class="mt-5 flex items-center justify-between gap-3">
                            <span class="brand-display text-3xl text-forest">{{ $product['formatted_price'] }}</span>
                            <button class="btn-primary px-4 py-2.5 text-xs disabled:pointer-events-none disabled:opacity-50" type="button" data-testid="product-add-button" x-on:click="window.dispatchEvent(new CustomEvent('cart-opening'))" wire:click="addToCart({{ (int) $product['id'] }})" wire:loading.attr="disabled" @disabled(! $isAvailable)>
                                <span class="inline-flex items-center gap-2"><span>+</span>{{ __('home.products.cta') }}</span>
                            </button>
                        </div>
                    </div>
                </article>
